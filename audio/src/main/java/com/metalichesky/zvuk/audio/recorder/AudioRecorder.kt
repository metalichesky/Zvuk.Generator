package com.metalichesky.zvuk.audio.recorder
...
import com.metalichesky.zvuk.audio.transformer.SimpleResampler
import java.io.File
...
class AudioRecorder {
    ...
    fun prepare(context: Context) {
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            throw IllegalStateException("Request record audio permission first")
        }
        val audioFile = audioFile ?: throw IllegalStateException("Set audio file first")
    ...
    private fun resample() {
        resamplingFinished = false
        while ((recorderThread != null && recorderThread?.isAlive == true) || recordedQueue.isNotEmpty()) {
    ...
    private fun convert() {
        val audioFile = audioFile ?: throw IllegalStateException("Audio file was not set")
        try {
            ...
            val muxer = try {
                ...
                throw IllegalStateException("Media muxer not created")
            }
            this.muxer = muxer

            val encoder = try {
                MediaCodec.createEncoderByType(ENCODER_TYPE)
            } catch (ex: Exception) {
                throw IllegalStateException("Encoder not created", ex)
            }
            ...
            do {
                ...
                do {
                    ...
                    if ((inputBufIndex != -1) && hasMoreData) {
                        ...
                        if (inputBufIndex >= 0) {
                            val dstBuf = encoder.getInputBuffer(inputBufIndex) ?: continue
                            dstBuf.clear()
                    ...
                    while (outputBufIndex != MediaCodec.INFO_TRY_AGAIN_LATER) {
                        if (outputBufIndex >= 0) {
                            val encodedData = encoder.getOutputBuffer(outputBufIndex) ?: continue
    ...
    private fun resampleAudio() {
        val tempFile = tempFile ?: throw IllegalStateException("Temp file wasn't set")
        ...
        this.tempFile = File(lastName)
    }

    private fun putAudioToContainer() {
        val audioFile = audioFile ?: throw IllegalStateException("Audio file wasn't set")
        if (!audioFile.exists()) {
            audioFile.createNewFile()
        }
        try {
            ...
            val muxer = try {
                ...
                throw IllegalStateException("Error creating media muxer", ex)
            }
            this.muxer = null
            ...
            val encoder = try {
                ...
                throw IllegalStateException("Error creating media encoder", ex)
            }
            this.encoder = encoder
